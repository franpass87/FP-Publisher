...
$tests = array(
	'uninstall_removes_plugin_options_and_transients' => function () {
		...
		$GLOBALS['tts_test_transients'] = array(
			'tts_rate_limit_facebook_hourly_123' => array(
				'value'   => 10,
				'expires' => time() + 100,
			),
			'tts_trello_boards_' . md5( 'demo' ) => array(
				'value'   => array(),
				'expires' => time() + 100,
			),
			'keep_transient'                     => array(
				'value'   => 1,
				'expires' => time() + 100,
			),
		);
		$GLOBALS['tts_test_site_transients'] = array(
			'tts_dashboard_stats' => array(
				'value'   => array(),
				'expires' => time() + 100,
			),
		);
		...
		tts_assert_false( isset( $GLOBALS['tts_test_options']['tts_settings'] ), 'Core settings option should be removed.' );
                ...
                tts_assert_false( isset( $GLOBALS['tts_test_options']['tts_plugin_version'] ), 'Stored plugin versions should be removed.' );
                ...
                tts_assert_true( in_array( 'tts_plugin_version', $GLOBALS['tts_deleted_site_options'], true ), 'Stored plugin version should be deleted from network options.' );
	...
	'uninstall_clears_cron_and_deletes_posts'         => function () {
		...
		$GLOBALS['tts_test_posts'][55]      = (object) array(
			'ID'        => 55,
			'post_type' => 'tts_client',
		);
		$GLOBALS['tts_test_client_posts'][] = (object) array(
			'ID'        => 55,
			'post_type' => 'tts_client',
		);
