...
if ( ! defined( 'TSAP_PLUGIN_BASENAME' ) ) {
	define( 'TSAP_PLUGIN_BASENAME', 'trello-social-auto-publisher/trello-social-auto-publisher.php' );
}
...
if ( ! function_exists( 'is_multisite' ) ) {
	function is_multisite() {
		return ! empty( $GLOBALS['tsap_test_network_mode'] );
	}
}
...
if ( ! function_exists( 'get_site_option' ) ) {
	function get_site_option( $option, $default = false ) {
		return $GLOBALS['tsap_test_network_options'][ $option ] ?? $default;
	}
}

if ( ! function_exists( 'update_site_option' ) ) {
	function update_site_option( $option, $value ) {
		$GLOBALS['tsap_test_network_options'][ $option ] = $value;
		return true;
	}
}
...
if ( ! function_exists( 'delete_site_option' ) ) {
	function delete_site_option( $option ) {
		...
		if ( isset( $GLOBALS['tsap_test_network_options'][ $option ] ) ) {
			unset( $GLOBALS['tsap_test_network_options'][ $option ] );
			$deleted = true;
		}

		if ( isset( $GLOBALS['tts_deleted_site_options'] ) && is_array( $GLOBALS['tts_deleted_site_options'] ) ) {
...
if ( ! function_exists( 'tsap_option_uses_network_storage' ) ) {
	function tsap_option_uses_network_storage( $option ) {
		return 0 === strpos( (string) $option, 'tts_' );
	}
}
...
if ( ! function_exists( 'tsap_get_option' ) ) {
	function tsap_get_option( $option, $default = false ) {
		if ( tsap_should_use_network_option( $option ) && array_key_exists( $option, $GLOBALS['tsap_test_network_options'] ) ) {
			return $GLOBALS['tsap_test_network_options'][ $option ];
		}
...
if ( ! function_exists( 'tsap_update_option' ) ) {
	function tsap_update_option( $option, $value, $autoload = null ) {
		if ( tsap_should_use_network_option( $option ) ) {
			$GLOBALS['tsap_test_network_options'][ $option ] = $value;
...
if ( ! function_exists( 'tsap_delete_option' ) ) {
	function tsap_delete_option( $option ) {
		...
		if ( tsap_should_use_network_option( $option ) && isset( $GLOBALS['tsap_test_network_options'][ $option ] ) ) {
			unset( $GLOBALS['tsap_test_network_options'][ $option ] );
			$deleted = true;
		}

		if ( isset( $GLOBALS['tts_test_options'][ $option ] ) ) {
			unset( $GLOBALS['tts_test_options'][ $option ] );
			$deleted = true;
		}
