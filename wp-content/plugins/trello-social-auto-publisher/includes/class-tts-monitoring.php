...
class TTS_Monitoring {
	...
	public static function perform_health_check() {
		...
		// Store health data
		tsap_update_option( 'tts_last_health_check', $health_data );
	...
	private static function send_health_alerts( $alerts ) {
		$alert_settings = tsap_get_option( 'tts_alert_settings',
			array(
				'enabled'            => false,
		...
		$high_priority_alerts = array_filter(
			$alerts,
			function ( $alert ) {
		...
		$message .= "Please check your Social Auto Publisher dashboard for more details.\n";
                $message .= 'Dashboard: ' . admin_url( 'admin.php?page=fp-publisher-dashboard' );
	...
	public static function generate_daily_report() {
		...
		// Store report
		tsap_update_option( 'tts_daily_report_' . current_time( 'Y_m_d' ), $daily_report );
	...
	public static function show_health_warnings() {
		...
		if ( ! empty( $high_priority_alerts ) ) {
			...
			echo '<p><strong>Social Auto Publisher Health Alert:</strong> ' . count( $high_priority_alerts ) . ' critical issue(s) detected.</p>';
                        echo '<p><a href="' . admin_url( 'admin.php?page=fp-publisher-dashboard' ) . '">View Dashboard</a></p>';
			echo '</div>';
		}
	}
