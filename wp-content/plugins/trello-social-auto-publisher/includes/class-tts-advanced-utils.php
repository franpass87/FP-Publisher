...
class TTS_Advanced_Utils {
	...
	public static function export_data( $options = array() ) {
		...
		try {
			...
			if ( $options['settings'] ) {
				$export_data['data']['settings'] = tsap_get_option( 'tts_settings', array() );
			}
	...
	public static function import_data( $import_data, $options = array() ) {
		...
		try {
			...
			// Import settings
			if ( isset( $data['settings'] ) && ( $options['overwrite_settings'] || ! tsap_get_option( 'tts_settings' ) ) ) {
				tsap_update_option( 'tts_settings', $data['settings'] );
				$import_log[] = 'Settings imported successfully';
			}

			// Import social apps (excluding secrets)
			if ( isset( $data['social_apps'] ) && ( $options['overwrite_social_apps'] || ! tsap_get_option( 'tts_social_apps' ) ) ) {
				$current_apps     = tsap_get_option( 'tts_social_apps', array() );
				...
				tsap_update_option( 'tts_social_apps', $current_apps );
