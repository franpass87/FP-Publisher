...
class TTS_Rate_Limiter {
	...
	private function apply_custom_limits() {
		$config = tsap_get_option( 'tts_channel_limits', array() );
	...
	private function log_request( $request_log ) {
		$logs = tsap_get_option( 'tts_api_request_logs', array() );
		...
		$logs[] = $request_log;
		tsap_update_option( 'tts_api_request_logs', $logs );
	}
	...
	public function get_rate_limit_analytics( $platform = null, $days = 7 ) {
		...
		$filtered_logs = array_filter(
			$logs,
			function ( $log ) use ( $platform, $cutoff_time ) {
	...
	public function cleanup_expired_limits() {
		...
		if ( count( $filtered_logs ) !== count( $logs ) ) {
			tsap_update_option( 'tts_api_request_logs', array_values( $filtered_logs ) );
		}
