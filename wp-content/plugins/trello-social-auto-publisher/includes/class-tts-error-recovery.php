...
class TTS_Error_Recovery {
	...
	private function queue_for_retry( $error_data, $operation, $context, $strategy, $severity ) {
		...
		TTS_Logger::log(
			sprintf(
				'Operation queued for retry: %s (attempt %d/%d)',
				$operation,
				$retry_count,
				$this->max_retries
			),
	...
	private function handle_permanent_failure( $error_data, $operation, $context, $severity ) {
		...
		TTS_Logger::log(
			sprintf(
				'Permanent failure for operation: %s. Error: %s',
				$operation,
				$error_data['message']
			),
			'error'
	...
	private function log_error( $error_data, $operation, $context, $severity ) {
		...
		$error_logs   = tsap_get_option( 'tts_error_logs', array() );
		...
		tsap_update_option( 'tts_error_logs', $error_logs );
	}
	...
	public function ajax_clear_retry_queue() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
            ...
            tsap_delete_option( $this->retry_queue_option );
