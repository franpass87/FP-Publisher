...
class TTS_Security_Audit {
	...
	public function monitor_admin_access() {
		...
		if ( ! current_user_can( 'manage_options' ) && ! current_user_can( 'edit_posts' ) ) {
			...
			} elseif ( in_array( $script_name, $allowed_scripts, true ) ) {
					$should_log_violation = false;
	...
	private function auto_block_ip( $ip, $event_type ) {
		...
		tsap_update_option( 'tts_blocked_ips', $blocked_ips );
	...
	private function get_active_blocked_ips() {
		$stored = tsap_get_option( 'tts_blocked_ips', array() );
		...
		if ( $dirty ) {
			tsap_update_option( 'tts_blocked_ips', $normalized );
		}
	...
	public function get_security_stats( $days = 7 ) {
		...
		return array(
			...
			'blocked_ips_count' => count( tsap_get_option( 'tts_blocked_ips', array() ) ),
		);
	}
