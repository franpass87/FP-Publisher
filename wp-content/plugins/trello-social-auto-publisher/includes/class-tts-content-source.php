...
add_action( 'save_post_tts_social_post', array( 'TTS_Content_Source', 'clear_source_stats_cache' ), 10, 0 );
add_action( 'delete_post', array( 'TTS_Content_Source', 'maybe_clear_source_stats_cache' ), 10, 1 );
add_action( 'trashed_post', array( 'TTS_Content_Source', 'maybe_clear_source_stats_cache' ), 10, 1 );
add_action( 'untrashed_post', array( 'TTS_Content_Source', 'maybe_clear_source_stats_cache' ), 10, 1 );
...
class TTS_Content_Source {
        ...
        /**
         * Cache key for aggregated source statistics.
         */
        const SOURCE_STATS_CACHE_KEY = 'tts_source_stats_v1';

        /**
         * Cache lifetime for aggregated source statistics (10 minutes).
         */
        const SOURCE_STATS_CACHE_TTL = 600;
	...
	public static function is_sync_available( $source ) {
		switch ( $source ) {
			case 'trello':
				if ( ! tsap_get_option( 'tts_trello_enabled', 1 ) ) {
					return false;
				}
	 ...
	 */
        public static function get_source_stats() {
                $cache_key = self::SOURCE_STATS_CACHE_KEY;
             ...
             GROUP BY meta_value"
                ...
                set_transient( $cache_key, $stats, self::SOURCE_STATS_CACHE_TTL );

                return $stats;
        }

        /**
         * Clear cached source statistics.
         */
        public static function clear_source_stats_cache() {
                delete_transient( self::SOURCE_STATS_CACHE_KEY );
        }

        /**
         * Clear cached source statistics when relevant posts change state.
         *
         * @param int $post_id Post identifier.
         */
        public static function maybe_clear_source_stats_cache( $post_id ) {
                if ( 'tts_social_post' !== get_post_type( $post_id ) ) {
                        return;
                }

                self::clear_source_stats_cache();
        }
	...
	private static function get_google_drive_settings() {
		$settings = tsap_get_option( 'tts_google_drive_settings', array() );
		...
		if ( '' === $access_token ) {
			$access_token = trim( (string) tsap_get_option( 'tts_google_drive_access_token', '' ) );
		}

		if ( '' === $folder_id ) {
			$folder_id = trim( (string) tsap_get_option( 'tts_google_drive_folder_id', '' ) );
		}
	...
	private static function get_dropbox_settings() {
		$settings = tsap_get_option( 'tts_dropbox_settings', array() );
		...
		if ( '' === $access_token ) {
			$access_token = trim( (string) tsap_get_option( 'tts_dropbox_access_token', '' ) );
		}

		if ( '' === $folder_path ) {
			$folder_path = trim( (string) tsap_get_option( 'tts_dropbox_folder_path', '' ) );
		}
		...
		wp_send_json_success(
			array(
				...
				/* translators: %d: number of synced items */
					__( 'Synced %1$d items from %2$s', 'fp-publisher' ),
					$synced_count,
					self::SOURCES[ $source ]
				),
			)
